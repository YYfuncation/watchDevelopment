...
- (void)applicationDidFinishLaunching {
    ...
    [unCenter requestAuthorizationWithOptions:UNAuthorizationOptionBadge | UNAuthorizationOptionSound | UNAuthorizationOptionAlert completionHandler:^(BOOL granted, NSError * _Nullable error) {
        if (error) {
            NSLog(@"请求通知权限错误：%@", [error localizedDescription]);
        }
        if (granted) {
            // allow notification
...
- (void)userNotificationCenter:(UNUserNotificationCenter *)center didReceiveNotificationResponse:(UNNotificationResponse *)response withCompletionHandler:(void (^)(void))completionHandler {
    ...
    if ([category isEqualToString:@"myLocationCategory"]) {
        // 本地通知
        
    } else if ([category isEqualToString:@"myRemoteCategory"]) {
        // 远程通知
